...
Thanks to cross-platform compatibility of Python, most PyMS functionality
works seamlessly on Windows. The only exception is reading of data
in NetCDF format which is not supported on Windows because, because
the 'pycdf' package is not supported on Windows (see the Subsection
\ref{subsec:pycdf}).

We use Linux for the development and deployment PyMS, and this User
Guide largely assumes that PyMS is used under Linux. The exception
is this Subsection, where we give some pointers on how to use PyMS
under Windows.
